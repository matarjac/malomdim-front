...
const AddSubTopicModalBox: React.FC<IModalBox> = (props: IModalBox) => {
    ...
    const [studyMaterialsState, setStudyMaterialsState] = useState([{ title: '', link: '', type: ContentTypes.Videos }]);

    const addAnotherMaterial = async () => {
        setStudyMaterialsState(
            [...studyMaterialsState, { title: '', link: '', type: ContentTypes.Videos }]
        )
    }
        ...
        <>
            {props.isShown &&
                <ModalOverLay onClick={handleClose}>
                    <ModalBoxContent width={50} height={75} onClick={(e) => e.stopPropagation()}>
                        ...
                        <AddSubTopicButton onClick={addAnotherMaterial}><img src="./icons/green-plus-icon.svg" alt="" />
                        </AddSubTopicButton>

                        <div style={{ display: 'flex', flexDirection: 'column', overflow: 'scroll', gap: '10px', height: '250px' }}>
                            ...
                            (
                                <div key={index} style={{
                                    ...
                                    <StyledInput placeholder="Title" style={{ width: '50%' }} onChange={(e) => { updateMaterialTitle(index, e.target.value) }} />
                                    <StyledInput placeholder="URL" style={{ width: '70%' }} onChange={(e) => { updateMaterialLink(index, e.target.value) }} />
                                    <FilterButton isSelected={studyMaterialsState[index].type == ContentTypes.Videos} onClick={() => { updateMaterialType(index, ContentTypes.Videos) }}>video</FilterButton>
                                    <FilterButton isSelected={studyMaterialsState[index].type == ContentTypes.Links} onClick={() => { updateMaterialType(index, ContentTypes.Links) }}>link</FilterButton>

                                </div>)
                            )}
                        </div>
                        <StyledAddButton onClick={() => { handleAddSubTopic({ title: newSubTopicTitle, materials: studyMaterialsState }) }} style={{ marginTop: '10px' }}>Add SubTopic</StyledAddButton>
                    </ModalBoxContent>
                </ModalOverLay>
            }
        </>
